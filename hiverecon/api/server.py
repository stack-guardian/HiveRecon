...
async def run_scan_background(
    ...
    """Actually run the scan using HiveMindCoordinator."""
    config = get_config()
...
@app.get("/scans/{scan_id}/summary", tags=["Scans"])
async def get_scan_summary(scan_id: str, session: AsyncSession = Depends(get_db)):
